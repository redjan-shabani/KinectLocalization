...
Matcher::Matcher(int32_t param_nms_n,
                 ...
                 int32_t param_multi_stage,
                 int32_t param_half_resolution,
                 int32_t param_refinement) : param_nms_n(param_nms_n),
                                             ...
                                             param_multi_stage(param_multi_stage),
                                             param_half_resolution(param_half_resolution),
                                             param_refinement(param_refinement) {
  ...
  if (param_half_resolution)
    this->param_match_radius /= 2;
}
...
void Matcher::pushBack (uint8_t *I1,uint8_t* I2,const int32_t width,const int32_t height,const int32_t bpl,const bool replace) {
...
void Matcher::computeFeatures (uint8_t *I,const int32_t* dims,int32_t* &max1,int32_t &num1,int32_t* &max2,int32_t &num2,uint8_t* &I_du,uint8_t* &I_dv,uint8_t* &I_du_full,uint8_t* &I_dv_full) {
  ...
  if (param_multi_stage) {
    int32_t nms_n_sparse = param_nms_n*3;
    if (nms_n_sparse>10)
      nms_n_sparse = max(param_nms_n,10);
  ...
  nonMaximumSuppression(I_f1,I_f2,dims_matching,maxima2,param_nms_n);
...
void Matcher::computePriorStatistics (vector<Matcher::p_match> &p_matched,int32_t method) {
  ...
  int32_t u_bin_num = (int32_t)ceil((float)dims_c[0]/(float)param_match_binsize);
  int32_t v_bin_num = (int32_t)ceil((float)dims_c[1]/(float)param_match_binsize);
  ...
  for (vector<Matcher::p_match>::iterator it=p_matched.begin(); it!=p_matched.end(); it++) {
    ...
    int32_t u_bin_min = min(max((int32_t)floor(it->u1c/(float)param_match_binsize)-1,0),u_bin_num-1);
    int32_t u_bin_max = min(max((int32_t)floor(it->u1c/(float)param_match_binsize)+1,0),u_bin_num-1);
    int32_t v_bin_min = min(max((int32_t)floor(it->v1c/(float)param_match_binsize)-1,0),v_bin_num-1);
    int32_t v_bin_max = min(max((int32_t)floor(it->v1c/(float)param_match_binsize)+1,0),v_bin_num-1);
  ...
  for (int32_t v_bin=0; v_bin<v_bin_num; v_bin++) {
    for (int32_t u_bin=0; u_bin<u_bin_num; u_bin++) {
      ...
      delta delta_min(-param_match_radius);
      delta delta_max(+param_match_radius);
...
inline void Matcher::findMatch (int32_t* m1,const int32_t &i1,int32_t* m2,const int32_t &step_size,vector<int32_t> *k2,
  ...
  // otherwise: use full search space
    ...
    u_min = u1-param_match_radius;
    u_max = u1+param_match_radius;
    v_min = v1-param_match_radius;
    v_max = v1+param_match_radius;
  }
  
  // if stereo search => constrain to 1d
  if (!flow) {
    v_min = v1-param_match_disp_tolerance;
    v_max = v1+param_match_disp_tolerance;
  }
  
  // bins of interest
  int32_t u_bin_min = min(max((int32_t)floor(u_min/(float)param_match_binsize),0),u_bin_num-1);
  int32_t u_bin_max = min(max((int32_t)floor(u_max/(float)param_match_binsize),0),u_bin_num-1);
  int32_t v_bin_min = min(max((int32_t)floor(v_min/(float)param_match_binsize),0),v_bin_num-1);
  int32_t v_bin_max = min(max((int32_t)floor(v_max/(float)param_match_binsize),0),v_bin_num-1);
...
void Matcher::matching (int32_t *m1p,int32_t *m2p,int32_t *m1c,int32_t *m2c,
  ...
  if (method==0) {
    ...
    for (i1c=0; i1c<n1c; i1c++) {
      ...
      int32_t u_bin = min((int32_t)floor((float)u1c/(float)param_match_binsize),u_bin_num-1);
      int32_t v_bin = min((int32_t)floor((float)v1c/(float)param_match_binsize),v_bin_num-1);
...
void Matcher::removeOutliers (vector<Matcher::p_match> &p_matched,int32_t method) {
  ...
  for (int32_t i=0; i<out.numberoftriangles; i++) {
    ...
    if (method==0) {
      ...
      if (fabs(p1_flow_u-p2_flow_u)+fabs(p1_flow_v-p2_flow_v)<param_outlier_flow_tolerance) {
        num_support[p1]++;
        num_support[p2]++;
      }

      // consistency of 2. edge
      if (fabs(p2_flow_u-p3_flow_u)+fabs(p2_flow_v-p3_flow_v)<param_outlier_flow_tolerance) {
        num_support[p2]++;
        num_support[p3]++;
      }

      // consistency of 3. edge
      if (fabs(p1_flow_u-p3_flow_u)+fabs(p1_flow_v-p3_flow_v)<param_outlier_flow_tolerance) {
        num_support[p1]++;
        num_support[p3]++;
      }
    ...
    // method: stereo
      ...
      // consistency of 1. edge
      if (fabs(p1_disp-p2_disp)<param_outlier_disp_tolerance) {
        num_support[p1]++;
        num_support[p2]++;
      }

      // consistency of 2. edge
      if (fabs(p2_disp-p3_disp)<param_outlier_disp_tolerance) {
        num_support[p2]++;
        num_support[p3]++;
      }

      // consistency of 3. edge
      if (fabs(p1_disp-p3_disp)<param_outlier_disp_tolerance) {
        num_support[p1]++;
        num_support[p3]++;
      }
    ...
    // method: quad matching
      ...
      // consistency of 1. edge
      if (fabs(p1_disp-p2_disp)<param_outlier_disp_tolerance && fabs(p1_flow_u-p2_flow_u)+fabs(p1_flow_v-p2_flow_v)<param_outlier_flow_tolerance) {
        num_support[p1]++;
        num_support[p2]++;
      }

      // consistency of 2. edge
      if (fabs(p2_disp-p3_disp)<param_outlier_disp_tolerance && fabs(p2_flow_u-p3_flow_u)+fabs(p2_flow_v-p3_flow_v)<param_outlier_flow_tolerance) {
        num_support[p2]++;
        num_support[p3]++;
      }

      // consistency of 3. edge
      if (fabs(p1_disp-p3_disp)<param_outlier_disp_tolerance && fabs(p1_flow_u-p3_flow_u)+fabs(p1_flow_v-p3_flow_v)<param_outlier_flow_tolerance) {
        num_support[p1]++;
        num_support[p3]++;
      }
    }
