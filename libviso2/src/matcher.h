...
public:
    ...
    Matcher(int32_t param_nms_n                  = 4,    // non-max-suppression: min. distance between maxima (in pixels)
            int32_t param_nms_tau                = 50,   // non-max-suppression: interest point peakiness threshold
            int32_t param_match_binsize          = 50,   // matching bin width/height (affects efficiency only)
            int32_t param_match_radius           = 200,  // matching radius (du/dv in pixels)
            int32_t param_match_disp_tolerance   = 1,    // du tolerance for stereo matches (in pixels)
            int32_t param_outlier_disp_tolerance = 5,    // outlier removal: disparity tolerance (in pixels)
            int32_t param_outlier_flow_tolerance = 5,    // outlier removal: flow tolerance (in pixels)
            int32_t param_multi_stage            = 1,    // 0=disabled,1=multistage matching (denser and faster)
            int32_t param_half_resolution        = 0,    // 0=disabled,1=match at half resolution, refine at full resolution
            int32_t param_refinement             = 1);   // refinement (0=none,1=pixel,2=subpixel)
    ...
    //        width,height ... image width and height (both images must be rectified and of same size)
    //        bpl ............ bytes per line (often equals width)
    ...
    void pushBack (uint8_t *I1,uint8_t* I2,const int32_t width,const int32_t height,const int32_t bpl,const bool replace);
    ...
    void pushBack (uint8_t *I1,const int32_t width,const int32_t height,const int32_t bpl,const bool replace) { pushBack(I1,0,width,height,bpl,replace); }
