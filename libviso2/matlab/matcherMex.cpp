...
void mexFunction (int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
  ...
  if (!strcmp(command,"init")) {
    ...
    if (nrhs!=0+1 && nrhs!=10+1)
      mexErrMsgTxt("0 or 10 inputs required (param_nms_n,param_nms_tau,param_match_binsize,param_match_radius,param_match_disp_tolerance,param_outlier_disp_tolerance,param_outlier_flow_tolerance,param_half_resolution,param_subpix_refinement).");
    ...
    // read parameters
      ...
      for (int32_t i=1; i<1+10; i++)
        if (!mxIsDouble(prhs[i]) || mxGetM(prhs[i])*mxGetN(prhs[i])!=1)
            mexErrMsgTxt("Input parameters must be double scalars.");

      M = new Matcher( (int32_t)*((double*)mxGetPr(prhs[ 1])),
                       (int32_t)*((double*)mxGetPr(prhs[ 2])),
                       (int32_t)*((double*)mxGetPr(prhs[ 3])),
                       (int32_t)*((double*)mxGetPr(prhs[ 4])),
                       (int32_t)*((double*)mxGetPr(prhs[ 5])),
                       (int32_t)*((double*)mxGetPr(prhs[ 6])),
                       (int32_t)*((double*)mxGetPr(prhs[ 7])),
                       (int32_t)*((double*)mxGetPr(prhs[ 8])),
                       (int32_t)*((double*)mxGetPr(prhs[ 9])),
                       (int32_t)*((double*)mxGetPr(prhs[10])));
    }
  ...
  // push back stereo image pair
    ...
    if (nrhs==1+1) {
      ...
      M->pushBack(I1,dims1[0],dims1[1],dims1[0],!strcmp(command,"replace"));
    ...
    // push back stereo image pair
      ...
      // compute features and put them to ring buffer
      M->pushBack(I1,I2,dims1[0],dims1[1],dims1[0],!strcmp(command,"replace"));
    }
  ...
  // get matches
  } else if (!strcmp(command,"getmatches")) {
